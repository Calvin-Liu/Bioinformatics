...
def hash_read(read, genome_ht):
    ...
    start_counter = Counter() #Counter can give you highest count and add things
    ...
    if best_alignment_count < 2: #if it shows 1 or 0 times, were gonna say it doesnt align
        return -1
    else:
...
def hash_genome(reference, key_length):
    """
    defaultdict: If you look up something that is not there, no exception is thrown
    ref_piece: key length piece of the reference genome
    ...
    for i in range(len(reference) - key_length):
        ...
        genome_hash[ref_piece].append(i) #looking up a same length piece in the hash table
        #for instance if ref piece is 4, break read into 4 and then look each one up
    return genome_hash
